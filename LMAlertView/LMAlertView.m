...
- (void)setupWithTitle:(NSString *)title message:(NSString *)message cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitles:(NSArray *)otherButtonTitles; {
    ...
    if (title != nil) {
        self.titleLabel = [self createLabel:title sideMargin:sideMargin labelWidth:labelWidth yOffset:yOffset];
    ...
    if (message != nil) {
        self.messageLabel = [self createLabel:title sideMargin:sideMargin labelWidth:labelWidth yOffset:yOffset];
        self.message = self.messageLabel.text;
...
// 创建一个自动适应高度和宽度的Label
- (UILabel *)createLabel:(NSString *)title sideMargin:(CGFloat)sideMargin labelWidth:(CGFloat)labelWidth yOffset:(CGFloat)yOffset {
    UILabel *label = [[UILabel alloc] init];
    label.numberOfLines = 0;
    label.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    label.text = title;

    CGSize sizeThatFits = [label sizeThatFits:CGSizeMake(labelWidth, MAXFLOAT)];
    label.frame = CGRectMake(sideMargin, yOffset, labelWidth, sizeThatFits.height);
